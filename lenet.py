...
weights = {
    'conv_1': tf.Variable(tf.truncated_normal(
        [5, 5, 1, layer_width['conv_1']])),
    'conv_2': tf.Variable(tf.truncated_normal(
        [5, 5, layer_width['conv_1'], layer_width['conv_2']])),
    'fc_1': tf.Variable(tf.truncated_normal(
        [5*5*16, layer_width['fc_1']])),
    'fc_2': tf.Variable(tf.truncated_normal(
        [layer_width['fc_1'], layer_width['fc_2']]))
}

biases = {
    'conv_1': tf.Variable(tf.zeros(layer_width['conv_1'])),
    'conv_2': tf.Variable(tf.zeros(layer_width['conv_2'])),
    'fc_1': tf.Variable(tf.zeros(layer_width['fc_1'])),
    'fc_2': tf.Variable(tf.zeros(layer_width['fc_2'])),
}
...
def LeNet(x):
    ...
    x = tf.nn.conv2d(x, weights['conv_1'], strides=[1, 1, 1, 1], padding='VALID')
    x = tf.nn.bias_add(x, biases['conv_1'])
    x = tf.nn.relu(x)
    x = tf.nn.max_pool(x, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

    x = tf.nn.conv2d(x, weights['conv_2'], strides=[1, 1, 1, 1], padding='VALID')
    x = tf.nn.bias_add(x, biases['conv_2'])
    x = tf.nn.relu(x)
    x = tf.nn.max_pool(x, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

    x = flatten(x)

    x = tf.add(tf.matmul(x, weights['fc_1']), biases['fc_1'])
    x = tf.nn.relu(x)

    x = tf.add(tf.matmul(x, weights['fc_2']), biases['fc_2'])
